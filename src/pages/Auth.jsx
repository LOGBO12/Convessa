...
import { signInWithGoogle, signInWithGithub, sendPhoneOTP } from '../services/authService';
...
const Auth = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Envoyer le code OTP via WhatsApp
    const result = await sendPhoneOTP(cleanPhone, true);
    ...
    if (result.success) {
      ...
      navigate('/verify-otp', { 
        state: { 
          phone: cleanPhone,
          devOtp: result.devOtp // Code OTP en mode dev
        } 
      });
    } else {
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 border-2 border-gray-100">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              Connectez-vous ou créez un compte en quelques secondes
            </p>
          </div>
          ...
          {error && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl flex items-center space-x-2"
            >
              <AlertCircle size={18} />
